...
import fr.o80.bitriseplugin.ui.formatter.InstantFormatter
...
class BuildPanel(
    ...
    private val content = JPanel(VerticalLayout(1)).apply {
        ...
        val startDateStr = instantFormatter.format(branch.moreRecentBuild.startDate)

        add(JLabel(branch.ref).bold().tooltip(branch.ref))
        add(JComment("$startDateStr - $duration").tooltip("$since ago"))
